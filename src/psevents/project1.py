...
def find_booked_slots(venue, date, venue_time, games_count, event_time, debug=True):
    # cursor = db.cursor()
    venues = [(i["open_time"], i["close_time"]) for i in venue_time]
#     print(venues)
    games_avail_count = games_count[0]["games_total_count"]

    booked_slots = []
    event_date = datetime.datetime.strptime(date, '%Y-%m-%d').date()
    open_time = venues[0][0]
    close_time = venues[0][1]
    open_time = datetime.datetime.strptime(open_time, '%H').time()
    close_time = datetime.datetime.strptime(close_time, '%H').time()

    event_times = [(i["start_time"], i["end_time"]) for i in event_time]
#     print(event_times)
    ...
    #     assert start <= end, "Cannot attend all appointments"
        while start + duration <= end:
            if games_avail_count!=0 :
                if debug==True:
                    part1[(start.strftime('%H:%M'), (start + duration).strftime('%H:%M'))] = games_avail_count[0]
                    # print(part1)
